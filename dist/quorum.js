 ...
 */
export function quorumState(witnesses, attestations, opts = {}) {
    ...
    if (quorumThreshold > quorumSize) {
        throw new Error(`kotoba-datomic: quorumThreshold ${quorumThreshold} exceeds quorumSize ${quorumSize}`);
    }
    if (witnesses.length !== quorumSize) {
        throw new Error(`kotoba-datomic: witness set has ${witnesses.length} cells, expected quorumSize ${quorumSize}`);
    }
    ...
    const byVerdict = { accept: [], reject: [], escalate: [] };
    for (const a of finalAttestations)
        byVerdict[a.verdict].push(a);
 ...
 * return.
 */
export async function collectQuorum(witnesses, source, opts = {}) {
    const quorumSize = opts.quorumSize ?? 5;
    ...
    while (true) {
        ...
        if (state.kind !== "pending")
            return state;
        if (collected.length >= quorumSize)
            break;
    }
