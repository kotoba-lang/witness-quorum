...
export async function selectWitnesses(
  ...
  if (fleet.length < quorumSize) {
    throw new Error(
      `kotoba-datomic: fleet has ${fleet.length} cells, cannot select quorum of ${quorumSize}`,
    );
  }
  if (quorumSize < 1) {
  ...
  for (let i = 0; selected.length < quorumSize; i++) {
    ...
    if (i >= sorted.length * 2) {
      throw new Error(
        `kotoba-datomic: witness selection loop did not converge — fleet may have duplicate keys`,
      );
    }
  }
  return selected;
